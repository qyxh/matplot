...
from .prompt import ERROR_PROMPT, INITIAL_SYSTEM_PROMPT, INITIAL_USER_PROMPT, VIS_SYSTEM_PROMPT, VIS_USER_PROMPT, ZERO_SHOT_COT_PROMPT,INITIAL_USER_PROMPT_CSV,VIS_USER_PROMPT_CSV
...
class PlotAgent():
    ...
    def __init__(self, config, original_query, query, code='', data_information=None,json_csv=''):
        self.chat_history = []
    ...
    def generate(self, model_type, query_type, file_name):
        ...
        if query_type == 'initial':
            messages = []
            messages.append({"role": "system", "content": fill_in_placeholders(INITIAL_SYSTEM_PROMPT, information1)})
            messages.append({"role": "user", "content": fill_in_placeholders(INITIAL_USER_PROMPT, information1)})
            print(messages)
        else:
            messages = []
            messages.append({"role": "system", "content": fill_in_placeholders(VIS_SYSTEM_PROMPT, information2)})
            if self.csv:
                messages.append({"role": "user", "content": fill_in_placeholders(VIS_USER_PROMPT_CSV, information2)})
            else:
                messages.append({"role": "user", "content": fill_in_placeholders(VIS_USER_PROMPT, information2)})
            print(messages)
    ...
    def run(self, model_type, query_type, file_name):
        ...
        result = self.generate(model_type=model_type, query_type=query_type, file_name=file_name)
        logging.info('\nresult:\n')
        logging.info(result)
        logging.info('======================================================================================================')
        while try_count < 4:
            ...
            with open(os.path.join(self.workspace, file_name), 'w',encoding='utf-8') as f:
                f.write(code)
            error = None
    ...
    def run_initial(self, model_type, file_name):
        ...
        self.chat_history = []
        log, code = self.run(model_type, 'initial', file_name)
        return log, code

    def run_vis(self, model_type, file_name):
        print('========Plot AGENT Novice RUN========')
        self.chat_history = []
        log, code = self.run(model_type, 'vis_refined', file_name)
        return log, code
